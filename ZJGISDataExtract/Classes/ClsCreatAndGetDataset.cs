...
namespace ZJGISDataExtract
{
    class ClsCreatAndGetDataset
    {
        ...
        //** 修改日期:
        //** 修改时间: 20080818
        //** 参数列表: pFields (esrigeodatabase.IFields)
        //             pDesSpatialReference 创建的,即目标要素类的空间参考
        //** 版    本:1.0
        //*********************************************************************************
        //</CSCM>
        ...
        public static IFields GetFieldsByFields(IFields pFields, ref ISpatialReference pDesSpatialReference, Dictionary<string, string> pDicField)
        {
            ...
            for (i = 0; i <= pFields.FieldCount - 1; i++)
            {
                ...
                {
                    //for sde featureclass modified by yh 4/14/2009
                    //If pField.Name.Contains(".") Then
                    //    Dim dotIndex As Integer
                    //    dotIndex = pField.Name.LastIndexOf(".")
                    //    pField.Name = pField.Name.Substring(dotIndex + 1, pField.Name.Length - dotIndex - 1)
                    //End If
                    ...
                    if ((pDicField != null))
                    {
                        if (pDicField.ContainsKey(pField.Name.Trim()))
                        {
                            bIsAddField = true;
                            //修改字段的名称
                            pFieldEdit = (IFieldEdit)pCreateField;
                            pFieldEdit.Name_2 = pDicField[pField.Name.Trim()];
        ...
        /// 
        /// </summary>
        /// <param name="sPDBOrFDBRouteAndName">创建数据库的路径</param>
        /// <param name="bCreateFDB"></param>
        /// <returns></returns>

        public static IWorkspace GetPDBOrGDBWorkspace(string sPDBOrFDBRouteAndName, bool bCreateFDB)//-------------创建个人数据库-------------
        {
            //bCreateFDB = false;
            IWorkspace functionReturnValue = default(IWorkspace);
            ...
            if (string.IsNullOrEmpty(sPDBOrFDBRouteAndName.Trim()))
            {
                functionReturnValue = null;
                return functionReturnValue;
            }
            ...
            if (bCreateFDB)
            {
                pWorkspaceFactory = new FileGDBWorkspaceFactoryClass();//--------文件存储------
            }
            else
            {
                pWorkspaceFactory = new AccessWorkspaceFactoryClass();//---------Access存储
            }

            //获得文件名
          //  sFileName = Microsoft.VisualBasic.FileIO.FileSystem.GetName(sPDBOrFDBRouteAndName);
            sFileName=  System.IO.Path.GetFileNameWithoutExtension(sPDBOrFDBRouteAndName);
 
            //20110723  HSh 注释因为文件名称未加后缀名".gdb"而不能创建的问题
            //sFileName = sFileName.Substring(0, sFileName.Length - 4);

            //获得路径
           //sFilePath=  System.IO.Directory.GetDirectoryRoot(sPDBOrFDBRouteAndName);
           sFilePath = System.IO.Directory.GetParent(sPDBOrFDBRouteAndName).FullName;
           //sFilePath  = Microsoft.VisualBasic.FileIO.FileSystem.GetParentPath(sPDBOrFDBRouteAndName);
            ...
            if (bCreateFDB == false)
            {
                if (Directory.Exists(sPDBOrFDBRouteAndName) == false)
                {
